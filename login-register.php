...
<body>
  ...
  <div class="login-content">
    ...
    <div class="nk-block" id="l-forget-password">
      <div class="nk-form">
        <p class="text-left">Send Password to you email.</p>
        <div class="input-group">
          <span class="input-group-addon nk-ic-st-pro"><i class="notika-icon notika-mail"></i></span>
          <div class="nk-int-st">
  ...
  <script>
    ...
    function signIn() {
      if ($("#user_").val() == "") {
        ...
        $.ajax({
          type: 'POST',
          url: '<?= $CFG->src_auth; ?>/login',
          data: {
            user_ajax: $("#user_").val(),
            pass_ajax: $("#password_").val(),
          },
          success: function(response) {
            ...
            if (response == 'SUCCESS_LOGIN') {
              ...
              var nAnimIn = 'animated fadeInDown';
              var nAnimOut = 'animated fadeOutUp';
              var text_ = 'ผุ้ใช้งานถูกระงับการใช้งาน !';
    ...
    $('#showPassword').on('ifChanged', function() {
      $(this).is(':checked') ? $('#password_').attr('type', 'text') : $('#password_').attr('type', 'password');
    });
    ...
    //   $.get("<? //= $CFG->src_auth; ?>/get_dep.php?deptId=" + escape(deptId) + "", function(data) {
